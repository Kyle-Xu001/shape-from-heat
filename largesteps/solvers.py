...
import numpy as np
from cholespy import CholeskySolverF, MatrixType, CholeskySolverD
import torch
...
class CholeskySolver():
    ...
    def solve(self, b, backward=False):
        x = torch.zeros_like(b).to(dtype=torch.float32, device=b.device).contiguous()
        self.solver.solve(b.detach().contiguous(), x)
        return x

class CholeskyDSolver():
    """
    Cholesky solver.
    ...
    """
    def __init__(self, M):
        self.solver = CholeskySolverD(M.shape[0], M.indices()[0], M.indices()[1], M.values(), MatrixType.COO)
