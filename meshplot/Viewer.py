...
class Viewer():
    ...
    def __get_shading(self, shading):
        shad = {"flat":True, "wireframe":False, "wire_width": 0.03, "wire_color": "black",
                ...
                "bbox": False, "roughness": 0.5, "metalness": 0.05, "reflectivity": 1.0,
                "line_width": 1.0, "line_color": "black",
                "point_color": "red", "point_size": 0.01, "point_shape": "circle",
                "text_color" : "red", "opacity": 1.0,
               }
        for k in shading:
            shad[k] = shading[k]
        return shad
    ...
    def rotate_camera(self, azimuth=0.0, elevation=0.0):
        if len(self.__objects) == 0:
            return
        ma = np.zeros((len(self.__objects), 3))
        mi = np.zeros((len(self.__objects), 3))
        ...
        # Use rotateOnAxis method instead of Euler angles
        self._orbit.target = mean
        self._cam.lookAt(mean)

        azimuth = np.deg2rad(azimuth)
        elevation = np.deg2rad(elevation)
        self._cam.position = [mean[0] + scale * np.cos(elevation) * np.sin(azimuth),
                              mean[1] + scale * np.sin(elevation) * np.sin(azimuth),
                              mean[2] + scale * np.cos(elevation)]
        self._light.position = [mean[0] + scale * np.cos(elevation) * np.sin(azimuth),
                                mean[1] + scale * np.sin(elevation) * np.sin(azimuth),
                                mean[2] + scale * np.cos(elevation)]
    ...
    def get_renderer_stream(self):
        stream = ipywebrtc.WidgetStream(widget=self._renderer)
        return stream
    ...
    def add_points(self, points, c=None, shading={}, obj=None, **kwargs):
        ...
        m_attributes = {"size": sh["point_size"], "opacity": sh["opacity"]}
    ...
    def remove_object_type(self, obj_type):
        for obj_id in list(self.__objects.keys()).copy():
            if self.__objects[obj_id]["type"] == obj_type:
                self._scene.remove(self.__objects[obj_id]["mesh"])
                del self.__objects[obj_id]
        self.__update_view()
    ...
    def update_object(self, oid=0, vertices=None, colors=None, faces=None):
        ...
        if type(vertices) != type(None):
            ...
            if obj["wireframe"] is not None:
                obj["mesh"].remove(obj["wireframe"])
                wf_geometry = p3s.WireframeGeometry(obj['geometry']) # WireframeGeometry
                sh = self.__get_shading({})
                wf_material = p3s.LineBasicMaterial(color=sh["wire_color"], linewidth=sh["wire_width"])
                wireframe = p3s.LineSegments(wf_geometry, wf_material)
                obj["mesh"].add(wireframe)
                obj["wireframe"] = wireframe

 #           obj["geometry"].exec_three_obj_method('computeVertexNormals')
            #     obj["wireframe"].geometry.geometry.attributes["position"].array = v
            #     obj["wireframe"].geometry.geometry.attributes["position"].needsUpdate = True
...
#            obj["geometry"].exec_three_obj_method('computeVertexNormals')
        ...
        # if obj["wireframe"] is not None:
        #     # Wireframe setup
        #     wf_geometry = p3s.WireframeGeometry(obj["mesh"].geometry) # WireframeGeometry
        #     wf_material = p3s.LineBasicMaterial(color=sh["wire_color"], linewidth=sh["wire_width"])
        #     wireframe = p3s.LineSegments(wf_geometry, wf_material)
        #     mesh.add(wireframe)
        #     mesh_obj["wireframe"] = wireframe
...
#        self.scene.exec_three_obj_method('update')
    ...
    def add_text(self, text_str, pos=None, color='black', size=0.05):
        font = "helvetiker"
        geometry = p3s.TextGeometry(text=text_str, font=font, size=size)
        material = p3s.MeshBasicMaterial(color=color)
        text = p3s.Mesh(geometry=geometry, material=material)
        # Center text on top if pos is None
        if pos is None:
            pos = np.array([0, 0, 0])
            pos[1] = geometry.boundingBox.max.y/2
        text.position = pos
        text_obj = {"geometry": geometry, "mesh": text, "material": material,
                    "max": pos, "min": pos, "type": "Text", "wireframe": None}
        return self.__add_object(text_obj)
